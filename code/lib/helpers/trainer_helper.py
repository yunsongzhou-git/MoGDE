...
class Trainer(object):
    def __init__(self,
        ...
        if self.cfg_train.get('resume_model', None):
            ...
            # _ = load_checkpoint(self.model, self.optimizer, self.cfg_train['resume_model'], self.logger, map_location=self.device)
    ...
    def train(self):
        ...
        # self.eval_one_epoch()
        # loss_weights = {'seg_loss': torch.tensor(1), 'size2d_loss': torch.tensor(1), 'offset2d_loss': torch.tensor(1), 'offset3d_loss': torch.tensor(1), 'size3d_loss': torch.tensor(1), 'heading_loss': torch.tensor(1), 'depth_loss': torch.tensor(1), 'trans_loss': torch.tensor(1)}
            ...
            for batch_idx, (inputs, calibs, coord_ranges, _, info) in enumerate(self.test_loader):
                ...
                for key, val in info.items():
                    if key == 'img_id':
                        ...
                        info_new[key] = val.detach().cpu().numpy()
                info = info_new
    ...
    def save_results(self, results, output_dir='./outputs'):
        ...
        output_dir = os.path.join('/mnt/disk/KITTIOdometry_Full/KITTI_FLAT/data/output/test_origin_trans')
        os.makedirs(output_dir, exist_ok=True)
        
        progress_bar = tqdm.tqdm(total=len(results.keys()), leave=True, desc='Write File Progress')
