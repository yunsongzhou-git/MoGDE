...
class Hierarchical_Task_Learning:
    def __init__(self,epoch0_loss,stat_epoch_nums=5):
        ...
        self.loss_graph = {'seg_loss':[],
                           ...
                           'trans_loss':['size2d_loss','size3d_loss','offset2d_loss']}                                 
...
class GupnetLoss(nn.Module):
    ...
    def compute_bbox3d_loss(self, input, target, mask_type = 'mask_2d'):
        ...
        # print(mask_type)
        # print('depth', depth_input.shape, depth_target.shape)
        depth_loss = laplacian_aleatoric_uncertainty_loss(depth_input, depth_target, depth_log_variance)

        # compute trans loss     
        trans_input = input['trans_depth'][input['train_tag']] 
        trans_input, trans_log_variance = trans_input[:, 0:1], trans_input[:, 1:2]
        trans_target = extract_target_from_tensor(target['depth'], target[mask_type])
        trans_loss = laplacian_aleatoric_uncertainty_loss(trans_input, trans_target, trans_log_variance)
        ...
        heading_loss = compute_heading_loss(input['heading'][input['train_tag']] ,
                                            ...
                                            target['heading_bin'],
                                            target['heading_res'])
        loss = depth_loss + offset3d_loss + size3d_loss + heading_loss + trans_loss
        ...
        self.stat['trans_loss'] = trans_loss
